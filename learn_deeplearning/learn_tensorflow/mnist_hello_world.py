...
def foo2():
    ...
    n = 50   # 隐含层神经元个数
    lr = 0.01
    W1 = tf.Variable(tf.zeros([784,n]), name='W1')  # 隐含层1
    b1 = tf.Variable(tf.zeros([n]), name='b1')
    y1 = tf.nn.sigmoid(tf.matmul(x,W1) + b1) 

    W = tf.Variable(tf.zeros([n,10]), name='W')
    b = tf.Variable(tf.zeros([10]), name='b')
    y = tf.nn.softmax(tf.matmul(y1,W) + b) 

    y_ = tf.placeholder("float", [None,10], name='y_') # 真实值（标签）,在训练的时候要代入真实值的，所以用占位符
